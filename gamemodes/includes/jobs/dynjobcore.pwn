...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys) {
    if((newkeys & KEY_YES)) {
        for(new i; i < MAX_JOBPOINTS; ++i) {
			if(IsPlayerInRangeOfPoint(playerid, 3.0, JobData[i][jPos][0], JobData[i][jPos][1],JobData[i][jPos][2])) {
...
public UpdateJobPoint(id)
{
	...
	if(JobData[id][jPos][0] == 0.0 || JobData[id][jPos][0] == 0) return 1;
	...
	format(szMiscArray, sizeof szMiscArray, "{FFFF00}Job Point ({FFFFFF}ID: %i{FFFF00})\n\nTen cong viec: {FFFFFF}%s\n{FFFF00}Nhan phim {FFFFFF}~k~~CONVERSATION_YES~ {FFFF00}hoac go {FFFFFF}/xinviec {FFFF00}de nhan cong viec.", id, GetJobName(JobData[id][jType]));
...
Job_GetJob(playerid, i)
{
    ...
    {
        format(szMiscArray, sizeof(szMiscArray), "Ban can phai dat cap do %i de nhan cong viec nay.", JobData[i][jLevel]);
    ...
    if(PlayerInfo[playerid][pJob] == JobData[i][jType] || PlayerInfo[playerid][pJob2] == JobData[i][jType] || PlayerInfo[playerid][pJob3] == JobData[i][jType]) {
    	return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban da co cong viec nay roi ma.");
    }
	if(PlayerInfo[playerid][pJob] == 0) format(szMiscArray, sizeof(szMiscArray), "Ban co muon tiep tuc su nghiep la mot {FFFF00}%s{FFFFFF}?", GetJobName(JobData[i][jType]));
	else if(0 < PlayerInfo[playerid][pDonateRank] < 4) format(szMiscArray, sizeof(szMiscArray), "Ban co muon tiep tuc su nghiep la mot {FFFF00}%s{FFFFFF}? (VIP Job)", GetJobName(JobData[i][jType]));
	else if(PlayerInfo[playerid][pDonateRank] > 3) format(szMiscArray, sizeof(szMiscArray), "Ban co muon tiep tuc su nghiep la mot {FFFF00}%s{FFFFFF}? (Platinum VIP Job)", GetJobName(JobData[i][jType]));
	else if(PlayerInfo[playerid][pFamed] > 0) format(szMiscArray, sizeof(szMiscArray), "Ban co muon tiep tuc su nghiep la mot {FFFF00}%s{FFFFFF}? (OS/Famed Job)", GetJobName(JobData[i][jType]));
	else if(PlayerInfo[playerid][pJob] > 0 && PlayerInfo[playerid][pDonateRank] == 0 && PlayerInfo[playerid][pFamed] == 0) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban dang co mot cong viec san roi, su dung '/quitjob' de xoa cong viec dang co de co the nhan cong viec moi.");
	
	SetPVarInt(playerid, PVAR_JOB_OBTAINING, JobData[i][jType]);
	Dialog_Show(playerid, acceptjob, DIALOG_STYLE_MSGBOX, "Xin viec lam", szMiscArray, "Xac nhan", "Huy");
	return 1;
}
...
stock GetJobName(i)
{
	if(i >= MAX_JOBTYPES) {
		new string[32];
		format(string, sizeof(string), "Khong ro (ID job: %d)", i);
		return string;
	}
	else return JobName[i];
}

CMD:getjob(playerid, params[])
{
	SendClientMessageEx(playerid, COLOR_GRAD1, "Lenh nay da bi xoa. Nhan phim ~k~~CONVERSATION_YES~ khi dang dung gan dia diem nhan viec.");
	return 1;
}
...
//            SendClientMessageEx(playerid, COLOR_GRAD1, "--* Job Vehicle Commands --* '/createjobtype' - '/jobtypes' - '/createjobveh' - '/deletejobveh'");
    ...
    else SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong phai lauthorised to use this command.");
    return 1;
}
...
CMD:editjobpoint(playerid, params[]) {
	...
	else SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong phai lauthorized to do this!");
	return 1;
}
...
Dialog:acceptjob(playerid, response, listitem, inputtext[]) {
	...
	if(response) {
		...
		if(PlayerInfo[playerid][pJob] == 0)	{
			...
			SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* Chuc mung ban da nhan duoc cong viec moi, go /trogiup de xem cac lenh danh rieng cho no.");
			if(iJob == 21) {

				SendClientMessageEx(playerid, COLOR_WHITE, "Ban duoc tang mot dong phuc Pizza Stack!");
				SendClientMessageEx(playerid, COLOR_WHITE, "Ban da chap nhan tham gia Pizza Nation; mot trong nhung xa hoi bi an nhat the gioi.");
            	SendClientMessageEx(playerid, COLOR_WHITE, "Luu y: Dung dung vao Pizza, se phai doi mat voi mot cai chet dai, dau don , sap xay ra.");
		...
		if((PlayerInfo[playerid][pDonateRank] > 0 || PlayerInfo[playerid][pFamed] > 0) && PlayerInfo[playerid][pJob2] == 0) {
            ...
            SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* Chuc mung ban da nhan duoc cong viec moi, go /trogiup de xem cac lenh danh rieng cho no.");
            SendClientMessageEx(playerid, COLOR_YELLOW, "VIP: Ban da nhan cong viec thu 2.");

            if(iJob == 21) {
            	SendClientMessageEx(playerid, COLOR_WHITE, "Ban da duoc trao dong phuc Pizza!");
            	SendClientMessageEx(playerid, COLOR_WHITE, "Ban da chap nhan tham gia bang dang Pizza , mot trong nhung thanh nien vui nhon nhat the gioi");
            	SendClientMessageEx(playerid, COLOR_WHITE, "Ghi nho : Dung bao gio an hiep pizza , hoac se doi mat voi viec an dan , hoac dau don");
            	SetPlayerSkin(playerid, 155);
            	PlayerInfo[playerid][pModel] = 155;
            }
        ...
        if((PlayerInfo[playerid][pDonateRank] >= 3 || PlayerInfo[playerid][pFamed] > 0) && PlayerInfo[playerid][pJob3] == 0) {
		    ...
		    SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* Chuc mung ban da nhan duoc cong viec moi, go /trogiup de xem cac lenh danh rieng cho no.");
            SendClientMessageEx(playerid, COLOR_YELLOW, "VIP: Ban da nhan cong viec thu 3 .");
		...
		return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong the nhan them cong viec nao. Vui long su dung /nghiviec de huy cong viec.");
    }
    return 1;
}
...
/*
{
	if(PlayerInfo[playerid][pLevel] < JobData[i][jLevel]) 
    {
        format(szMiscArray, sizeof(szMiscArray), "Ban can phai level %i to get this job.", JobData[i][jLevel]);
        return SendClientMessageEx(playerid, COLOR_GRAD1, szMiscArray);
    }
    if(PlayerInfo[playerid][pJob] == JobData[i][jType] || PlayerInfo[playerid][pJob2] == JobData[i][jType] || PlayerInfo[playerid][pJob3] == JobData[i][jType]) {
    	return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban da co cong viec nay roi ma.");
...
/*CMD:createjobveh(playerid, params[]) {
	if(PlayerInfo[playerid][pAdmin] < 4 && PlayerInfo[playerid][pASM] < 1) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong duoc phep su dung lenh nay.");
...
{
	if(PlayerInfo[playerid][pAdmin] < 1337) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong duoc phep su dung lenh nay.");
	new iVehID;
...
CMD:xinviec(playerid, params[])
{
	for(new i; i < MAX_JOBPOINTS; ++i) {
		if(IsPlayerInRangeOfPoint(playerid, 3.0, JobData[i][jPos][0], JobData[i][jPos][1],JobData[i][jPos][2])) {
			SendClientMessageEx(playerid, -1, "[XIN VIEC] Xin viec thanh cong (Job ID: %d)", i);
			Job_GetJob(playerid, i);
			break;
		}
	}
