...
hook OnPlayerClickTextDraw(playerid, Text:clickedid) {
	...
	if(clickedid == Furniture_TD[13]) callcmd::furniture(playerid, "");	
	if(clickedid == Furniture_TD[14]) callcmd::furniturehelp(playerid, "");
	if(clickedid == Furniture_TD[16]) callcmd::furnitureresetpos(playerid, "");
	...
	switch(dialogid) {
		...
		case DIALOG_FURNITURE_BUYSELECT: {
			...
			if(response) {
				...
				if(PlayerInfo[playerid][pDonateRank] < arrFurnitureCatalog[i][fc_iVIP] && PlayerInfo[playerid][pAdmin] < 4) return SendClientMessageEx(playerid, COLOR_GRAD1, "Cap do VIP cua ban khong du de mua loai noi that nay.");
				...
				format(szMiscArray, sizeof(szMiscArray), "Ban co muon mua %s cho $%s va %s vat lieu?", GetFurnitureName(arrFurnitureCatalog[i][fc_iModelID]), number_format(GetFurniturePrice(arrFurnitureCatalog[i][fc_iModelID])), number_format(GetFurniturePrice(arrFurnitureCatalog[i][fc_iModelID]) / 10));
		...
		case DIALOG_FURNITURE_BUYCONFIRM: {
			...
			if(!response) {
				...
				if(iSlotID > GetMaxFurnitureSlots(playerid) || iSlotID == -1) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong con du o trong noi that.");
				
				new iPrice = GetFurniturePrice(iModelID);
				if(GetPlayerMoney(playerid) < iPrice) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong du tien de mua vat pham nay.");
				if(PlayerInfo[playerid][pMats] < floatround((iPrice / 10))) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong du vat lieu de lam.");
		...
		case DIALOG_FURNITURE_PAINT2: {
					...
					for(new i; i < sizeof(arrTextures); ++i) {
						...
						if(strfind(arrTextures[i][text_TXDName], inputtext, true) != -1 || strfind(arrTextures[i][text_TextureName], inputtext, true) != -1) {
							...
							if(x > MAX_SEARCH_TEXTURES) {
								...
								format(szMiscArray, sizeof(szMiscArray), "** Tim kiem cua ban chua hon %d ket qua. Do doi, phan toi da se duoc hien thi.", MAX_SEARCH_TEXTURES);
			...
			if(!GetPVarType(playerid, "textslot")) {
				...
				SendClientMessageEx(playerid, COLOR_YELLOW, "[Furniture]: {CCCCCC}Ban da son thanh cong do noi that.");
				return 1;
			}
		}
		case DIALOG_PERMITBUILDER: {
			...
			if(!GetPVarType(playerid, "PRMBLD")) {
				...
				if(!IsPlayerConnected(uPlayer)) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban da chi dinh id nguoi choi khong hop le.");
				...
				switch(listitem) {
					case 0: {
						if(PlayerInfo[playerid][pPhousekey] == INVALID_HOUSE_ID) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban chua co nha trong o trong nay."), DeletePVar(playerid, "PRMBLD");
						iHouseID = PlayerInfo[playerid][pPhousekey];
					}
					case 1: {
						if(PlayerInfo[playerid][pPhousekey2] == INVALID_HOUSE_ID) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban chua co nha trong o trong nay."), DeletePVar(playerid, "PRMBLD");
						iHouseID = PlayerInfo[playerid][pPhousekey2];
					}
					case 2: {
						if(PlayerInfo[playerid][pPhousekey3] == INVALID_HOUSE_ID) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong co nha trong slot nay."), DeletePVar(playerid, "PRMBLD");
						iHouseID = PlayerInfo[playerid][pPhousekey3];
					}
					default: return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban da chi dinh ID nha khong hop le."), DeletePVar(playerid, "PRMBLD");
				}
				...
				format(szMiscArray, sizeof(szMiscArray), "%s cap cho ban quyen xay dung trong nha cho ho Quyen su dung nha truoc day duoc thay the.", GetPlayerNameEx(playerid));
				SendClientMessageEx(giveplayerid, COLOR_YELLOW, szMiscArray);
				format(szMiscArray, sizeof(szMiscArray), "Ban da cap cho %s quyen xay dung trong ngoi nha cua ban.", GetPlayerNameEx(giveplayerid));
				SendClientMessageEx(playerid, COLOR_YELLOW, szMiscArray);
			}
		}
		case DIALOG_FURNITURE_DESCONFIRM: {
			...
			if(response) {
				...
				SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da pha huy thanh cong tat ca do dac trong nha.");
			}
			else SendClientMessageEx(playerid, COLOR_GRAD1, "Ban da tu bo pha huy tat ca do dac trong nha.");
		}
	}
	return 0;
}
...
public OnPlayerSelectDynamicObject(playerid, objectid, modelid, Float:x, Float:y, Float:z) {
	...
	if(GetPVarType(playerid, "copdestroyfur")) {
		...
		if(iHouseID != iData) return SendClientMessageEx(playerid, COLOR_GRAD1, "Mon vat pham nay khong co trong mat hang noi that.");
		...
		if(IsValidFurniture(iHouseID, i, 1)) {
			...
			format(szMiscArray, sizeof(szMiscArray), "** %s pha huy mot manh do noi that.", GetPlayerNameEx(playerid));
			SendGroupMessage(GROUP_TYPE_LEA, COLOR_DBLUE, szMiscArray);
		}
		else SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong chi dinh mot mon do noi that.");
	}
	if(GetPVarType(playerid, PVAR_FURNITURE)) {
		...
		iData = Streamer_GetIntData(STREAMER_TYPE_OBJECT, objectid, E_STREAMER_EXTRA_ID);
		if(iHouseID != iData) return SendClientMessageEx(playerid, COLOR_GRAD1, "Mon vat nay khong phai thuoc loai noi that.");
		for(i = 0; i < MAX_FURNITURE_SLOTS; ++i) if(HouseInfo[iHouseID][hFurniture][i] == objectid) break;

		if(GetPVarType(playerid, "SellFurniture")) { 

			SetPVarInt(playerid, PVAR_FURNITURE_SLOT, i);
			format(szMiscArray, sizeof(szMiscArray), "Ban co muon ban mon %s cho $%s?", 
...
FurnitureMenu(playerid, menu = 0) {
	...
	if(iHouseID == INVALID_HOUSE_ID) {
		...
		callcmd::furniture(playerid, "");
		return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong con o trong nha nua.");
	}
	if(!HousePermissionCheck(playerid, iHouseID)) {
		callcmd::furniture(playerid, "");
		return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong duoc phep sua do dac cua ngoi nha nay.");
	}
	switch(menu) {
		...
		case 2: { // Edit your own furniture.
			...
			if(isnull(szMiscArray)) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong co do dac nao.");
...
SellFurniture(playerid, iHouseID, iSlotID, iPrice) {
	...
	format(szMiscArray, sizeof(szMiscArray), "Ban da ban %s cho $%s", GetFurnitureName(GetDynamicObjectModel(HouseInfo[iHouseID][hFurniture][iSlotID])), number_format(iPrice));
	SendClientMessage(playerid, COLOR_YELLOW, szMiscArray);
...
FurniturePermit(playerid) {
	...
	ShowPlayerDialogEx(playerid, DIALOG_PERMITBUILDER, DIALOG_STYLE_INPUT, "Furniture | Permit Builder", "Nhap ten ID hoac ten nguoi ban muon cho phep xay dung nha cua ban.", "Select", "Cancel");
...
CMD:furnituresystem(playerid, params[]) {
	...
	if(FurnitureSystem) {
		FurnitureSystem = 0;
		format(szMiscArray, sizeof(szMiscArray), "{AA3333}GTN-Warning{FFFF00}: %s (ID %d) disabled the Furniture System", GetPlayerNameEx(playerid), playerid);
		ABroadCast(COLOR_YELLOW, szMiscArray, 2);
	}
	else {
		FurnitureSystem = 1;
		format(szMiscArray, sizeof(szMiscArray), "{AA3333}GTN-Warning{FFFF00}: %s (ID %d) enabled the Furniture System", GetPlayerNameEx(playerid), playerid);
		ABroadCast(COLOR_YELLOW, szMiscArray, 2);
	}
	return 1;
}
...
CMD:myslots(playerid, params[]) {
	...
	format(szMiscArray, sizeof(szMiscArray), "** Ban co %d slot noi that ban co the su dung.", GetMaxFurnitureSlots(playerid));
...
CMD:furniture(playerid, params[]) {
	...
	if(GetPVarType(playerid, PVAR_FURNITURE)) {
		...
		if(i == INVALID_HOUSE_ID) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban phai o trong nha.");
		if(!HousePermissionCheck(playerid, i)) return SendClientMessage(playerid, COLOR_GRAD1, "Ban khong duoc phep chinh sua do dac trong ngoi nha nay.");
...
CMD:setfurnitureslots(playerid, params[]) {
	...
	if(IsAdminLevel(playerid, ADMIN_HEAD, 0)) {
		...
		if(!IsPlayerConnected(uPlayer)) return SendClientMessageEx(playerid, COLOR_GRAD1, "Nguoi choi khong online.");
		if(!(0 < iAmount < MAX_FURNITURE_SLOTS)) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban da chi dinh so tien khong hop le.");
		
		PlayerInfo[uPlayer][pFurnitureSlots] = iAmount;

		format(szMiscArray, sizeof(szMiscArray), "Administrator %s set noi that cua ban vo o trong %d.", GetPlayerNameEx(playerid), iAmount);
		SendClientMessageEx(uPlayer, COLOR_YELLOW, szMiscArray);

		format(szMiscArray, sizeof(szMiscArray), "Ban dat %s's o trong do noi that vao %d.", GetPlayerNameEx(uPlayer), iAmount);
		SendClientMessageEx(playerid, COLOR_YELLOW, szMiscArray);

		format(szMiscArray, sizeof(szMiscArray), "Administrator %s dat %s's o trong noi that vao %d.", GetPlayerNameEx(playerid), GetPlayerNameEx(uPlayer), iAmount);
		ABroadCast(COLOR_YELLOW, szMiscArray, 2);

		format(szMiscArray, sizeof(szMiscArray), "%s dat %s's (%d) o trong noi that vao %d.", GetPlayerNameExt(playerid), GetPlayerNameExt(uPlayer), PlayerInfo[uPlayer][pId], iAmount);
...
CMD:destroyallfurniture(playerid, params[]) {
	...
	ShowPlayerDialogEx(playerid, DIALOG_FURNITURE_DESCONFIRM, DIALOG_STYLE_MSGBOX, "Xoa noi that", "{FFFFFF}Ban co chac muon xoa {FF0000}vinh vien {FFFFFF}noi that ngoi nha nay?", "Yes", "No");
	return 1;
}
...
CMD:unfurnishhouse(playerid, params[]) {
	...
	// if(!HousePermissionCheck(playerid, iHouseID)) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong the lam dieu nay trong ngoi nha nay.");
	if(HouseInfo[iHouseID][hOwnerID] != GetPlayerSQLId(playerid)) return SendClientMessageEx(playerid, COLOR_GRAD1, "Chi nguoi so huu can nha co the lam dieu nay.");
	...
	if(fHouseZ == 0) return SendClientMessageEx(playerid, COLOR_GRAD1, "Nhiem vu nay khong the hoan thanh cho loai noi that ngoi nha cua ban.");

	if(HouseInfo[iHouseID][hInteriorZ] > fHouseZ + 25.0) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ngoi nha cua ban da bi hu hong.");
	...
	foreach(new p : Player) {
		...
		if(PlayerInfo[p][pVW] == PlayerInfo[playerid][pVW] && ProxDetectorS(50, playerid, p)) {
			...
			SendClientMessageEx(p, COLOR_GRAD1, "Ban dang duoc chuyen den phien ban nha khong noi that.");
...
CMD:furnishhouse(playerid, params[]) {
	...
	if(fHouseZ == 0) return SendClientMessageEx(playerid, COLOR_GRAD1, "Cong cu nay khong the hoan thanh cho loai noi that nha cua ban.");
	if(HouseInfo[iHouseID][hInteriorZ] < fHouseZ + 25.0) return SendClientMessageEx(playerid, COLOR_GRAD1, "Nha cua ban da duoc trang tri noi that.");
	...
	foreach(new p : Player) {
		...
		if(PlayerInfo[p][pVW] == PlayerInfo[playerid][pVW] && ProxDetectorS(50, playerid, p)) {
			...
			SendClientMessageEx(p, COLOR_GRAD1, "Ban duoc chuyen den phien ban nha chua co noi that.");
			defer Furniture_HousePosition(p, iHouseID);
		}
	}
	return 1;
...
CMD:furnitureresetpos(playerid, params[]) {
	...
	if(GetPVarType(playerid, PVAR_FURNITURE)) {
		...
		if(i == INVALID_HOUSE_ID) {
			...
			callcmd::furniture(playerid, "");
			return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban phai o trong nha.");
		}

		new iHouseID = GetPVarInt(playerid, PVAR_INHOUSE);
		Player_StreamPrep(playerid, HouseInfo[iHouseID][hInteriorX], HouseInfo[iHouseID][hInteriorY], HouseInfo[iHouseID][hInteriorZ], FREEZE_TIME);
	}
	else SendClientMessageEx(playerid, COLOR_GRAD1, "Ban chi duoc su dung cai nay khi ban duoc nga dung vi tri cua loai noi that do.");
	return 1;
}
...
CMD:destroyfurniture(playerid, params[]) {
	...
	if(iHouseID == INVALID_HOUSE_ID) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong co o trong nha");
	if(sscanf(params, "d", iSlotID)) return SendClientMessageEx(playerid, COLOR_GREY, "Usage: /destroyfurniture [slot].");
	if(!IsValidFurniture(iHouseID, iSlotID, 1)) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban da chi dinh vi tri khong hop le.");
	DestroyFurniture(iHouseID, iSlotID);
	return 1;
}
...
CMD:copdestroy(playerid, params[]) {
	...
	new iHouseID = GetHouseID(playerid);
	if(!IsACop(playerid)) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong phai la canh sat.");
	if(iHouseID == INVALID_HOUSE_ID) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong o trong nha");
...
CMD:revokebuilders(playerid, params[]) {
	...
	switch(iHouseID) {
		...
		default: return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban da chi dinh id khong hop le.");
	}
...
CMD:door(playerid, params[]) {
	...
	if(GetHouseID(playerid) == INVALID_HOUSE_ID) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban phai o trong nha.");
	...
	if(textm_Selected3DTextureMenu[playerid] != -1 || PlayerTextureMenuInfo[playerid][ptextm_TPreviewState] == PREVIEW_STATE_SELECT) {
		...
		if(PlayerTextureMenuInfo[playerid][ptextm_TPreviewState] == PREVIEW_STATE_SELECT) {
			...
			if(newkeys == KEY_LOOK_BEHIND) { // Cancel
				...
				DeletePVar(playerid, PVAR_FURNITURE_SLOT);
				DeletePVar(playerid, PVAR_FURNITURE_EDITING);
				SendClientMessageEx(playerid, COLOR_GRAD1, "** Ban da dung ket cau doi tuong.");
				PlayerTextDrawHide(playerid, Furniture_PTD[playerid][2]);
				PlayerTextDrawHide(playerid, Furniture_PTD[playerid][3]);
...
OnPlayerKeyStateChangeMenu(playerid,newkeys,oldkeys) {
	...
	else if(newkeys & KEY_ANALOG_LEFT || (((newkeys & (KEY_WALK | KEY_CTRL_BACK)) == (KEY_WALK | KEY_CTRL_BACK)) && ((oldkeys & (KEY_WALK | KEY_CTRL_BACK)) != (KEY_WALK | KEY_CTRL_BACK))))
	{
	    ...
	    if(PlayerTextureMenuInfo[playerid][ptextm_TPreviewState] == PREVIEW_STATE_ALLTEXTURES) {
			...
			if(addt >= 0) SendClientMessage(playerid, COLOR_GRAD1, "Ban da them thanh cong ket cau cho chu de cua ban.");
			else if(addt == -1) SendClientMessage(playerid, COLOR_GRAD1, "Ket cau nay da duoc ton tai trong chu de cua ban.");
			else if(addt == -2) SendClientMessage(playerid, COLOR_GRAD1, "Ban khong the them nhieu ket cau cho chu de cua ban.");
			return 1;
		}
	}
