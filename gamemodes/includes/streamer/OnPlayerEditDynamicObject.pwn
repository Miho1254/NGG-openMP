...
public OnPlayerEditDynamicObject(playerid, objectid, response, Float:x, Float:y, Float:z, Float:rx, Float:ry, Float:rz) {
	...
	*/
	...
	if(GetPVarType(playerid, PVAR_FURNITURE_EDITING)) {
		...
		switch(response) {
			...
			case EDIT_RESPONSE_FINAL: {
				...
				// if(!strcmp(GetFurnitureName(iModelID), "None", true)) {
				// 	SendClientMessage(playerid, -1, "Noi that ban chinh sua khong hop le");
				// 	return 1;
				// }
				if(!IsValidFurniture(iHouseID, iSlotID, 1) || HouseInfo[iHouseID][hFurniture][iSlotID] == -1) {
					GetDynamicObjectPos(objectid, x, y, z);
					GetDynamicObjectRot(objectid, rx, ry, rz);
					SetDynamicObjectPos(objectid, x, y, z);
					SetDynamicObjectRot(objectid, rx, ry, rz);
					...
					format(szMiscArray, sizeof(szMiscArray), "[Noi that]: Object ban chinh sua khong hop le %s(%d).", GetFurnitureName(iModelID), iModelID);
					SendClientMessageEx(playerid, COLOR_YELLOW, szMiscArray);
					DeletePVar(playerid, PVAR_FURNITURE_EDITING);
					DeletePVar(playerid, PVAR_FURNITURE_SLOT);
					PlayerTextDrawSetPreviewModel(playerid, Furniture_PTD[playerid][0], PlayerInfo[playerid][pModel]);
				...
				format(szMiscArray, sizeof(szMiscArray), "[Noi that]: Ban da dieu chinh thanh cong %s(%d).", GetFurnitureName(iModelID), iModelID);
				SendClientMessageEx(playerid, COLOR_YELLOW, szMiscArray);
	...
	if(response == EDIT_RESPONSE_FINAL)
	{
		...
		{
			if(PlayerInfo[playerid][pAdmin] < 4 && PlayerInfo[playerid][pASM] < 1 && PlayerInfo[playerid][pShopTech] < 1) return SendClientMessageEx(playerid, COLOR_GREY, "Ban khong phai lauthorized to perform this action!");
	...
	if(GetPVarType(playerid, "editingsign")) {
		if(response == EDIT_RESPONSE_FINAL)
		{
			...
			{
				if(PlayerInfo[playerid][pAdmin] < 4 && PlayerInfo[playerid][pGangModerator] < 1) return SendClientMessageEx(playerid, COLOR_GREY, "Ban khong phai lauthorized to perform this action!");
