...
CMD:deletepoll(playerid, params[])
{
	if(PlayerInfo[playerid][pAdmin] >= 1337 || PlayerInfo[playerid][pPR] > 0)
	{
		...
		if(iPollID >= 0 && iPollID < MAX_POLLS)
		{
			...
			else SendClientMessageEx(playerid, COLOR_GRAD2, "This poll khong ton tai.");
		}
...
{
	new iPollID = -1;
	if(PlayerInfo[playerid][pConnectHours] < 2) return SendClientMessage(playerid, COLOR_GRAD2, "Ban khong coenough playing hours to vote in this poll.");
	for(new i = 0; i < MAX_POLLS; i++)
	{
	...
	else SendClientMessageEx(playerid, COLOR_GRAD2, "Ban khong dung tai range of any polling stations.");
	return 1;
}
...
CMD:gotopoll(playerid, params[]) {
    ...
    if(PlayerInfo[playerid][pAdmin] < 1337 && PlayerInfo[playerid][pPR] == 0) return SendClientMessage(playerid, COLOR_GRAD2, "Ban khong duoc phep su dung lenh nay.");
    ...
    if(PollInfo[iPollID][poll_iID] == -1) return SendClientMessage(playerid, COLOR_GRAD2, "This poll khong ton tai.");
