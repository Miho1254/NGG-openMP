...
CMD:meterhelp(playerid, params[]) return callcmd::parkingmeterhelp(playerid, params);

CMD:meterstatus(playerid, params[])
{
	new meterid, string[128];
	meterid = GetNearestParkingMeter(playerid);
	if(meterid == -1 || !IsPlayerInRangeOfPoint(playerid, 2.0, ParkingMeterInformation[meterid][MeterPosition][0], ParkingMeterInformation[meterid][MeterPosition][1], ParkingMeterInformation[meterid][MeterPosition][2])) return SendClientMessageEx(playerid, COLOR_GREY, "Ban khong dung tai range of a parking meter.");
...
{
	if(GetPlayerState(playerid) != PLAYER_STATE_DRIVER) return SendClientMessageEx(playerid, COLOR_GREY, "You must be the driver of chiec xe to rent a parking meter.");
	new vehicleid, engine, lights, alarm, doors, bonnet, boot, objective, meterid, string[128];
	meterid = GetNearestParkingMeter(playerid);
	if(meterid == -1) return SendClientMessageEx(playerid, COLOR_GREY, "Ban khong dung tai range of a parking meter.");
	...
	if(engine == VEHICLE_PARAMS_ON)
	{
		...
		format(string, sizeof(string), "{FF8000}** {C2A2DA}%s van chia khoa vao o va dong co dung lai.", GetPlayerNameEx(playerid));
...
CMD:renewmeter(playerid, params[])
{
	...
	meterid = GetNearestParkingMeter(playerid);
	if(meterid == -1 || !IsPlayerInRangeOfPoint(playerid, 2.0, ParkingMeterInformation[meterid][MeterPosition][0], ParkingMeterInformation[meterid][MeterPosition][1], ParkingMeterInformation[meterid][MeterPosition][2])) return SendClientMessageEx(playerid, COLOR_GREY, "Ban khong dung tai range of a parking meter.");
	if(ParkingMeterInformation[meterid][AssignedVehicle] == INVALID_VEHICLE_ID) return SendClientMessageEx(playerid, COLOR_GREY, "This parking meter is not currently occupied by chiec xe.");
	if(GetPlayerCash(playerid) < ParkingMeterInformation[meterid][MeterRate])
	{
	...
	GivePlayerCash(playerid, -ParkingMeterInformation[meterid][MeterRate]);
	Tax += ParkingMeterInformation[meterid][MeterRate];
	format(string, sizeof(string), "Ban da gia han thoi gian do xang cua minh them 5 phut cho $%s.", number_format(ParkingMeterInformation[meterid][MeterRate]));
	SendClientMessageEx(playerid, COLOR_WHITE, string);
	format(string, sizeof(string), "{FF8000}** {C2A2DA}%s lay ra mot it tien, gui no vao dong ho dau xe.", GetPlayerNameEx(playerid));
	ProxDetector(30.0, playerid, string, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE);
	ParkingMeters();
	return 1;
}
...
CMD:editmeterposition(playerid, params[])
{
	...
	{
		format(string, sizeof(string), "ID dong ho xe duoc chi dinh nam trong khoang tu 1 va %s.", number_format(MAX_PARKING_METERS - 1));
		SendClientMessageEx(playerid, COLOR_GREY, string);
		return 1;
	}
	if(ParkingMeterInformation[meterid][MeterActive] == 0) return SendClientMessageEx(playerid, COLOR_GREY, "Dong ho do xe duoc chi dinh hien khong hoat dong.");
	if(strlen(name) < 4 || strlen(name) > 6) return SendClientMessageEx(playerid, COLOR_GREY, "Loai dong ho do xe duoc chi dinh khong hop le. Types: Meter, Parked, ToMe.");
