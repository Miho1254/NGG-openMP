...
CMD:ad(playerid, params[]) return callcmd::approvedoorsale(playerid, params);
CMD:ds(playerid, params[]) return callcmd::denydoorsale(playerid, params);

CMD:doorsalehelp(playerid, params[])
{
	SendClientMessageEx(playerid, COLOR_GREEN,"_______________________TRO GIUP SELL DOOR_______________________");
	SendClientMessageEx(playerid, COLOR_GREY, "/selldoors [playerid] - De Sell Door cho nguoi choi khac.");
	SendClientMessageEx(playerid, COLOR_GREY, "/huy door - De tu choi Sell Door.");
	if(PlayerInfo[playerid][pAdmin] >= 4 || PlayerInfo[playerid][pASM] >= 1)
	{
		SendClientMessageEx(playerid, COLOR_GREY, "Admin Commands: /doorrequests - De xem danh sach cho phe duyet Sell Door.");
		SendClientMessageEx(playerid, COLOR_GREY, "Admin Commands: /doorsaleinfo - De xem thong tin Sell Door cua nguoi choi.");
		SendClientMessageEx(playerid, COLOR_GREY, "Admin Commands: /ad [playerid] - De chap nha don yeu cau Sell Door.");
		SendClientMessageEx(playerid, COLOR_GREY, "Admin Commands: /ds [playerid] - De tu choi don yeu cau Sell Door.");
		return 1;
	}
	return 1;
}
...
CMD:approvedoorsale(playerid, params[])
{
	...
	if(sscanf(params, "u", target)) return SendClientMessageEx(playerid, COLOR_GREY, "USAGE: /approvedoorsale [playerid]");
	if(!IsPlayerConnected(target)) return SendClientMessageEx(playerid, COLOR_GREY, "The specified player isn't connected.");
	if(gPlayerLogged{target} == 0) return SendClientMessageEx(playerid, COLOR_GREY, "The specified player isn't logged into their account.");
	if(DDSalePendingAdmin[target] == false) return SendClientMessageEx(playerid, COLOR_GREY, "The specified player does not have a pending dynamic door sale request.");
	...
	if(GetPlayerCash(DDSaleTarget[target]) < DDSalePrice[target])
	{
		...
		format(string, sizeof(string), "No transactions have been made however, as Ban khong cothe sufficient funds ($%s).", number_format(DDSalePrice[target]));
				...
				case true:
				{
					...
					switch(listitem)
					{
						...
						case 10:
						{
							...
							{
								format(string, sizeof(string), "Ban khong cothe sufficient funds for the transfer fine ($%s).", number_format(fine));
								SendClientMessageEx(playerid, COLOR_GREY, string);
								return 1;
							}
