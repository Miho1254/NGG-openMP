...
public SAN_Process_Cameraman(playerid, choice, channel)
{
	...
	switch(choice)
	{
		...
		case SAN_CAMERAMAN_VEHICLE: // Nifty stuff here: The camera is set on the wing/foot of the aircraft. I used the spectate function, and it's working brilliantly (it's really smooth).
		{
			...
			{
				SendClientMessage(playerid, COLOR_LIGHTBLUE, "* You must be in chiec xe to use this command!");
				return 1;
			}
		}
...
{
	if(!IsAReporter(playerid)) return SendClientMessage(playerid, COLOR_GRAD1, "     Ban khong phai la News Reporter.");
...
CMD:broadcast(playerid, params[])
{
	if(IsAReporter(playerid))
	{
		...
		if(!GetPVarType(playerid, "ChannelID_FMEM")) return callcmd::setchannel(playerid, "");
		if(broadcasting == 0)
		{
			broadcasting = 1;
	...
	{
		SendClientMessage(playerid, COLOR_GRAD1, "     Ban khong phai la News Reporter.");
	}
	return 1;
}
...
CMD:news(playerid, params[])
{
	...
	if(IsAReporter(playerid))
	{
		...
		{
			SendClientMessage(playerid, COLOR_GREY, "Ban khong dung tai a news van or chopper or in the studio.");
			return 1;
		}
	}
	else
	{
		SendClientMessage(playerid, COLOR_GREY, "Ban khong phai la News Reporter.");
	}
	return 1;
}
...
CMD:setchannel(playerid, params[])
{
	...
	else SendClientMessage(playerid, COLOR_GREY, "     Ban khong phai la News Reporter.");
	return 1;
}
...
CMD:mic(playerid, params[])
{
	...
	if(IsAReporter(playerid) && !GetPVarInt(playerid, "ChannelID_FMEM"))
	{
		...
		return callcmd::setchannel(playerid, "");
	}
...
{
	szMiscArray[0] = 0;
	if(!IsAReporter(playerid)) return SendClientMessage(playerid, COLOR_GREY, "     Ban khong phai la News Reporter."); 
	if(!SAN_RankCheck(playerid, 1)) return 1;
	if(shutdown == 1) return SendClientMessage(playerid, COLOR_WHITE, "The news system is currently shut down." );
	if(GetPVarType(playerid, "ChannelID_FMEM"))
	{
			...
			if (ProxDetectorS(5.0, playerid, giveplayerid))
			{
				...
				else
				{
					if(SANShows[GetPVarInt(playerid, "ChannelID_FMEM")][ChannelType] != 1)
					{
						...
						format(szMiscArray, sizeof(szMiscArray), "* Ban de nghi %s to participate in the TV Show.", GetPlayerNameEx(giveplayerid));
						SendClientMessage(playerid, COLOR_LIGHTBLUE, szMiscArray);
					...
					else
					{
						...
						format(szMiscArray, sizeof(szMiscArray), "* Ban de nghi %s to participate in the Radio Show.", GetPlayerNameEx(giveplayerid));
						SendClientMessage(playerid, COLOR_LIGHTBLUE, szMiscArray);
...
SAN_RankCheck(playerid, slot) {
	...
	SendClientMessage(playerid, COLOR_GRAD1, "Ban khong co quyen de su dung lenh nay.");
	return 0;
}
