...
public ShipmentConvo(playerid, iStage) {
	...
	switch(iStage) {
		case 1: {
			SendClientMessageEx(playerid, COLOR_YELLOW, "(cellphone) Unknown Caller noi: Ayo ese I've got a job for ya.");
			SetTimerEx("ShipmentConvo", 3000, false, "ii", playerid, 2);
		}

		case 2: {
			SendClientMessageEx(playerid, COLOR_YELLOW, "(cellphone) Unknown Caller noi: I need an overseas shipment taken care of homie.");
			SendClientMessageEx(playerid, COLOR_YELLOW, "(cellphone) Unknown Caller noi: Can I trust you with my shit?");
			SetTimerEx("ShipmentConvo", 5000, false, "ii", playerid, 3);
		}

		case 3: {
			SendClientMessageEx(playerid, COLOR_YELLOW, "(cellphone) Unknown Caller noi: I'm sending you the coordinates right now.");
			SendClientMessageEx(playerid, COLOR_YELLOW, "(cellphone) Unknown Caller noi: Get a boat and head over there ASAP!");
...
CMD:delivershipment(playerid, params[]) {
	...
	szMiscArray[0] = 0;
	if(!IsACriminal(playerid)) return SendClientMessageEx(playerid, COLOR_WHITE, "Ban khong dung tai a criminal organization.");
	if(!IsAGangShipmentTruck(iVehID) || !iVehID) return SendClientMessageEx(playerid, COLOR_WHITE, "Ban khong dung tai a shipment truck!");
...
{
	if(PlayerInfo[playerid][pAdmin] < 99999) return SendClientMessageEx(playerid, COLOR_GRAD2, "Ban khong duoc phep su dung lenh nay.");
	new val;
