...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]) {
	...
	switch(dialogid)
	{
		case DIALOG_GANGTAGS_INPUT:
		{
			...
			format(szMiscArray, sizeof(szMiscArray), "{AA3333}GTN-Warning{FFFF00}: %s (ID: %d) (T:%d) sprayed %s", GetPlayerNameEx(playerid), playerid, GetPVarInt(playerid, PVAR_GANGTAGID), inputtext);
...
CMD:tag(playerid, params[])
{
	...
	if(PlayerInfo[playerid][pRank] == 0) return SendClientMessage(playerid, COLOR_GRAD1, "Ban can phai at least rank 1 to tag.");
	...
	SendClientMessage(playerid, COLOR_GRAD1, "Ban khong dung gan a tag point.");
	return 1;
}
