	...
	switch(dialogid)
	{
		...
		case DIALOG_GOVGUN_SELL2:
		{
			if(response)
			{
				...
				new wepid = GetPVarInt(playerid, "_GovGun");
				if(arrGroupData[iGroupID][g_iBudget] < arrWeaponCosts[wepid]) return SendClientMessageEx(playerid, COLOR_GRAD1, "Chinh phu khong du tien de tra cho ban.");
				...
				mysql_tquery(MainPipeline, szMiscArray, "OnQueryFinish", "ii", SENDDATA_THREAD, playerid);
				szMiscArray[0] = 0;
				format(szMiscArray, sizeof(szMiscArray), "* %s da ban khau sung %s cua ho cho chinh phu.", GetPlayerNameEx(playerid), Weapon_ReturnName(wepid));
		...
		case ARMS_MENU: {
			if(!response) return SendClientMessageEx(playerid, COLOR_GREY, "Ban da thoat khoi menu.");
			...
			switch(listitem) {
				...
				case 1: {
					...
					if(PlayerInfo[playerid][pGunLic] > gettime()) return SendClientMessageEx(playerid, COLOR_GRAD2, "Ban da co giay phep su dung vu khi roi.");

					ShowPlayerDialogEx(playerid, APPLY_GUN_LIC, DIALOG_STYLE_MSGBOX, 
						"Xac nhan mua giay phep vu khi", 
						"Ban dang yeu cau xin so huu giay phep su dung vu khi tu chinh phu.\nBan se duoc kiem tra ly lich toi pham trong 3 tuan qua\nPhi ban phai tra la: $350,000", 
						"Chap thuan", 
						"Huy"
					);
				}
...
CMD:govarms(playerid, params[])
{
	...
	else SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong phai la Tong Thong.");
	return 1;
}
...
/*CMD:sellgovgun(playerid, params[])
{
	...
	if(iCount == 0) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong coany weapons that you can sell to the government.");
