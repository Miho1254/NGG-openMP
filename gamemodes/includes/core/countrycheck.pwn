	...
	if(response_code == 200) {
		...
		*/
		switch(resCode) {
			...
			case 3: {
				format(szMiscArray, sizeof(szMiscArray), "{AA3333}GTN-Warning{FFFF00}: %s (IP:%s) has failed their countrycode check and was auto-kicked.", GetPlayerNameEx(playerid), PlayerInfo[playerid][pIP]);
				...
				SendClientMessage(playerid, COLOR_RED, "Your account is not set to log in from your current country. Make an Admin Request on SAW Community forum");
				SetTimerEx("KickEx", 5000, 0, "i", playerid);
			}
			case 4: {
				format(szMiscArray, sizeof(szMiscArray), "{AA3333}GTN-Warning{FFFF00}: %s (IP:%s) has attempted to log in while locked by the Geosecurity system, and was auto-kicked.", GetPlayerNameEx(playerid), PlayerInfo[playerid][pIP]);
				ABroadCast(COLOR_YELLOW, szMiscArray, 2);
				...
				Log("logs/geocheck.log", szMiscArray);
				SendClientMessage(playerid, COLOR_RED, "Your account is locked due to numerous logins from incorrect countries. Make an Admin Request on SAW Community forum");
				SetTimerEx("KickEx", 5000, 0, "i", playerid);
			}
			default: {
