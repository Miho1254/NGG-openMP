...
public OnPlayerEnterDynArea(playerid, areaid) {
	...
	if(i < MAX_VEHICLES) {
		if(iVehEnterAreaID[i] == areaid) {
			// printf("DEBUG: Area %d was detected as chiec xe area.", areaid);
			SetPVarInt(playerid, "VEHA_ID", i);
		}
	}
	return 1;
...
{
	if(GetPVarType(playerid, "StreamPrep")) return SendClientMessageEx(playerid, COLOR_WHITE, "Bao khong the thuc hien thao tac ngay bay gio, hay doi mot chut...");
	if(gettime() < DoorTimer[playerid]) return SendClientMessageEx(playerid, COLOR_GREY, "Ban phai doi %d giay truoc khi muon vao trong.", DoorTimer[playerid]-gettime());
	if(DDoorsInfo[i][ddVIP] > 0 && PlayerInfo[playerid][pDonateRank] < DDoorsInfo[i][ddVIP]) 
	{
		SendClientMessageEx(playerid, COLOR_GRAD2, "Khong the vao, door nay chi danh cho VIP.");
		return 1;
	}
	
	if(DDoorsInfo[i][ddFamed] > 0 && PlayerInfo[playerid][pFamed] < DDoorsInfo[i][ddFamed]) {
		SendClientMessageEx(playerid, COLOR_GRAD2, "Ban khong the vao, you're not a high enough famed level.");
		return 1;
	}

	if(DDoorsInfo[i][ddDPC] > 0 && PlayerInfo[playerid][pDedicatedPlayer] < 2) {
		SendClientMessageEx(playerid, COLOR_GRAD2, "Ban khong the vao, Ban khong phai la high enough Dedicated Player.");
		return 1;
	}

	if(DDoorsInfo[i][ddAllegiance] > 0) {
		if(!(0 <= PlayerInfo[playerid][pMember] < MAX_GROUPS) || arrGroupData[PlayerInfo[playerid][pMember]][g_iAllegiance] != DDoorsInfo[i][ddAllegiance]) return SendClientMessageEx(playerid, COLOR_GRAD2, "Ban khong the vao, this door is nation restricted.");
	...
	if(DDoorsInfo[i][ddGroupType] > 0) {
		if(!(0 <= PlayerInfo[playerid][pMember] < MAX_GROUPS) || arrGroupData[PlayerInfo[playerid][pMember]][g_iGroupType] != DDoorsInfo[i][ddGroupType] && arrGroupData[PlayerInfo[playerid][pMember]][g_iAllegiance] != DDoorsInfo[i][ddAllegiance]) {
			return SendClientMessageEx(playerid, COLOR_GRAD2, "Door nay dang bi khoa.");
		}
		else if(PlayerInfo[playerid][pRank] < DDoorsInfo[i][ddRank]) return SendClientMessageEx(playerid, COLOR_GRAD2, "You are not high enough rank to enter this door.");
	}

	if(DDoorsInfo[i][ddFaction] != INVALID_GROUP_ID) {
		if(PlayerInfo[playerid][pMember] != DDoorsInfo[i][ddFaction]) return SendClientMessageEx(playerid, COLOR_GRAD2, "Khong the vao, door nay chi danh cho to chuc.");
		else if(PlayerInfo[playerid][pRank] < DDoorsInfo[i][ddRank]) return SendClientMessageEx(playerid, COLOR_GRAD2, "You are not high enough rank to enter this door.");
	}

	if(DDoorsInfo[i][ddAdmin] > 0 && PlayerInfo[playerid][pAdmin] < DDoorsInfo[i][ddAdmin]) {
		SendClientMessageEx(playerid, COLOR_GRAD2, "Ban khong the vao, Ban khong phai la high enough admin level.");
		return 1;
	}

	if(DDoorsInfo[i][ddWanted] > 0 && PlayerInfo[playerid][pWantedLevel] != 0) {
		SendClientMessageEx(playerid, COLOR_GRAD2, "Ban khong the vao, this door restricts those with wanted levels.");
		return 1;
	}

	if(DDoorsInfo[i][ddLocked] == 1) {
		return SendClientMessageEx(playerid, COLOR_GRAD2, "Door nay da bi khoa.");
	}
...
{
	if(GetPVarType(playerid, "StreamPrep")) return SendClientMessageEx(playerid, COLOR_WHITE, "Bao khong the thuc hien thao tac ngay bay gio, hay doi mot chut....");
	if(gettime() < DoorTimer[playerid]) return SendClientMessageEx(playerid, COLOR_GREY, "Ban phai doi %d giay truoc khi muon ra ngoai.", DoorTimer[playerid]-gettime());
	if(DDoorsInfo[i][ddVIP] > 0 && PlayerInfo[playerid][pDonateRank] < DDoorsInfo[i][ddVIP]) 
	{
		SendClientMessageEx(playerid, COLOR_GRAD2, "Khong the vao, door nay chi danh cho VIP.");
		return 1;
	...
	if(DDoorsInfo[i][ddDPC] > 0 && PlayerInfo[playerid][pRewardHours] < 150) {
		SendClientMessageEx(playerid, COLOR_GRAD2, "Ban khong the vao, Ban khong phai la Dedicated Player.");
		return 1;
	}
	...
	if(DDoorsInfo[i][ddGroupType] > 0) {
		if(!(0 <= PlayerInfo[playerid][pMember] < MAX_GROUPS) || arrGroupData[PlayerInfo[playerid][pMember]][g_iGroupType] != DDoorsInfo[i][ddGroupType] && arrGroupData[PlayerInfo[playerid][pMember]][g_iAllegiance] != DDoorsInfo[i][ddAllegiance]) {
			return SendClientMessageEx(playerid, COLOR_GRAD2, "Khong the vao, door nay chi danh cho to chuc.");
		}
		else if(PlayerInfo[playerid][pRank] < DDoorsInfo[i][ddRank]) return SendClientMessageEx(playerid, COLOR_GRAD2, "You are not high enough rank to enter this door.");
	}
	...
	if(DDoorsInfo[i][ddAdmin] > 0 && PlayerInfo[playerid][pAdmin] < DDoorsInfo[i][ddAdmin]) {
		SendClientMessageEx(playerid, COLOR_GRAD2, "Ban khong the vao, Ban khong phai la Admin.");
		return 1;
	}
...
House_Enter(playerid, i) {
	...
	if(PlayerInfo[playerid][pPhousekey] == i || PlayerInfo[playerid][pPhousekey2] == i || HouseInfo[i][hLock] == 0 || PlayerInfo[playerid][pRenting] == i) {
		if(gettime() < DoorTimer[playerid]) return SendClientMessageEx(playerid, COLOR_GREY, "Ban phai doi %d truoc muon khi vao trong.", DoorTimer[playerid]-gettime());
		if(GetPVarType(playerid, "StreamPrep")) return SendClientMessageEx(playerid, COLOR_WHITE, "Ban khong the lam dieu nay ngay bay gio.");
		House_VistorCheck(i);
...
House_Exit(playerid, i) {
	...
	if(GetPVarType(playerid, "StreamPrep")) return SendClientMessageEx(playerid, COLOR_WHITE, "Ban khong the lam dieu nay ngay bay gio.");
	if(gettime() < DoorTimer[playerid]) return SendClientMessageEx(playerid, COLOR_GREY, "Ban phai doi %d truoc khi muon ra ngoai.", DoorTimer[playerid]-gettime());
	if(GetPVarType(playerid, PVAR_FURNITURE)) callcmd::furniture(playerid, "");
	House_VistorCheck(i);
	SetPlayerInterior(playerid,0);
	PlayerInfo[playerid][pInt] = 0;
...
{
	if(Businesses[i][bExtPos][1] == 0.0) return SendClientMessageEx(playerid, COLOR_WHITE, "Ban khong the vao cua hang nay.");
	if(Businesses[i][bStatus]) {
		if(GetPVarType(playerid, "StreamPrep")) return SendClientMessageEx(playerid, COLOR_WHITE, "Ban khong the lam dieu nay ngay bay gio.");
		if(gettime() < DoorTimer[playerid]) return SendClientMessageEx(playerid, COLOR_GREY, "Ban phai doi %d giay truoc khi co the ra khoi door nay.", DoorTimer[playerid]-gettime());
		if (Businesses[i][bType] == BUSINESS_TYPE_GYM)
		{
			...
			{
				GameTextForPlayer(playerid, "~r~Ban can co tien de vao Gym", 5000, 1);
				return 1;
			}
		}
		...
		{
			new string[50];
			format(string, sizeof(string), "Ban da vao phong Gym va phi tap la $%i.", Businesses[i][bGymEntryFee]);
			...
			{
				SendClientMessageEx(playerid, COLOR_WHITE, "(/batdauboi) de bat dau boi.");
				SendClientMessageEx(playerid, COLOR_WHITE, "(/thamgiaboxing) de xep hang dau boxing.");
			}
...
Garage_Enter(playerid, i) {
	...
	if(GarageInfo[i][gar_Locked] == 1) return SendClientMessageEx(playerid, COLOR_GRAD2, "Nha de xe hien dang bi khoa.");
	if(GetPVarType(playerid, "StreamPrep")) return SendClientMessageEx(playerid, COLOR_WHITE, "Ban khong the lam dieu nay. Wait for streaming to finish.");
	if(gettime() < DoorTimer[playerid]) return SendClientMessageEx(playerid, COLOR_GREY, "Ban phai doi %d vai giay truoc khi co the vao canh cua nay.", DoorTimer[playerid]-gettime());
	PlayerInfo[playerid][pVW] = GarageInfo[i][gar_InteriorVW];
...
Garage_Exit(playerid, i) {
	if(GetPVarType(playerid, "StreamPrep")) return SendClientMessageEx(playerid, COLOR_WHITE, "Ban khong the lam dieu nay. Wait for streaming to finish.");
	if(gettime() < DoorTimer[playerid]) return SendClientMessageEx(playerid, COLOR_GREY, "Ban phai doi %d vai giay truoc khi thoat khoi canh cua nays.", DoorTimer[playerid]-gettime());
