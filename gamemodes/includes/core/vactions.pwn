...
{
	//if(!IsPlayerIdle(playerid)) return SendClientMessageEx(playerid, COLOR_WHITE, "Ban phai dung hanh dong nay moi co the thuc hien hanh dong khac!");
...
IsAblePedAnimation(playerid)
{
    ...
    {
		SendClientMessage(playerid, COLOR_GRAD2, "Ban khong the thuc hien cac hanh dong khi dang ngoi tren xe, ban can phai xuong xe truoc.");
		return 0;
	}
	return 1;
}
...
IsAbleVehicleAnimation(playerid)
{
    ...
    {
		SendClientMessage(playerid, COLOR_GRAD2, "This animation requires you to be inside chiec xe.");
		return 0;
	}
	return 1;
}
...
LoadVactionsHelper()
{
	...
	"~w~Nhan ~r~~k~~PED_SPRINT~ ~w~de dung hanh dong");
...
public StopaniTimer(playerid)
{
	...
	{
	    SendClientMessageEx (playerid, COLOR_YELLOW, "That bai trong viec huy lenh hanh dong vi ban da chuyen");
    	for(new i = 0; i < 3; i++)
		{
...
{
	SendClientMessage(playerid, COLOR_GREEN, "____________ TAT CA LENH HANH DONG ____________");
	...
	SendClientMessage(playerid, COLOR_GREEN, "Su dung /stopani de dung toan bo hanh dong.");
	return 1;
}
...
CMD:stopani(playerid, params[])
{
	...
	else
	{
		...
		SendClientMessageEx (playerid, COLOR_YELLOW, "Khong duoc di chuyen trong 10 giay khi lenh hanh dong ket thuc!");
	}
	return 1;
}
...
CMD:shakehand(playerid, params[])
{
	...
	if(sscanf(params, "ud", giveplayerid, style)) return SendClientMessageEx(playerid, COLOR_GREY, "SU DUNG: /shakehand [player] [style (1-8)]");

	if(IsPlayerConnected(giveplayerid) || (PlayerInfo[giveplayerid][pAdmin] >= 2 && PlayerInfo[playerid][pTogReports] == 0))
	{
		if(giveplayerid == playerid)
		{
			SendClientMessageEx(playerid, COLOR_GREY, "Ban khong the lam dieu nay voi chinh ban.");
			return 1;
		}
		else if(PlayerInfo[giveplayerid][pAdmin] >= 2 && PlayerInfo[giveplayerid][pTogReports] != 1) {
			SendClientMessageEx(playerid, COLOR_GREY, "Ban khong the bat tay voi nguoi nay.");
			return 1;
		}
		if(style >= 1 && style < 9)
		{
			...
			{
				SendClientMessageEx(playerid, COLOR_GREY, "Ban khong o gan nguoi nay.");
				return 1;
			}
			...
			format(string, sizeof(string), "Ban da de nghi bat tay voi %s, hay cho ho phan hoi.", GetPlayerNameEx(giveplayerid));
			SendClientMessageEx(playerid, COLOR_LIGHTBLUE, string);

			format(string, sizeof(string), "%s da de nghi bat tay voi ban, su dung (/chapnhan battay) de chap nhan.", GetPlayerNameEx(playerid));
...
{
	if(GetPVarInt(playerid, "Injured") != 0 || PlayerCuffed[playerid] != 0 || GetPVarInt(playerid, "pBagged") >= 1 || PlayerInfo[playerid][pHospital] != 0) return SendClientMessageEx(playerid, COLOR_GRAD2, "Ban khong the lam dieu nay ngay bay gio.");
	
	new string[128], mtext[20], thour, suffix[3], year, month,day;
    getdate(year, month, day);
    if(month == 1) { mtext = "1"; }
    else if(month == 2) { mtext = "2"; }
    else if(month == 3) { mtext = "3"; }
    else if(month == 4) { mtext = "4"; }
    else if(month == 5) { mtext = "5"; }
    else if(month == 6) { mtext = "6"; }
    else if(month == 7) { mtext = "7"; }
    else if(month == 8) { mtext = "8"; }
    else if(month == 9) { mtext = "9"; }
	...
	{
		format(string, sizeof(string), "~y~%s, %s/%d/%d~n~~g~|~w~%d:%02d~g~%s|~n~~w~Thoi gian ra tu: ~r~%s", GetWeekday(1), mtext, day, year, thour, minuite, suffix, TimeConvert(PlayerInfo[playerid][pJailTime]));
	}
	else
	{
...
/*CMD:stopani(playerid, params[])
{
	...
	if(IsPlayerInAnyVehicle(playerid) == 1)
	{
		SendClientMessage(playerid, COLOR_GRAD2, "This command requires you to be outside chiec xe.");
		return 1;
	}
	ClearAnimationsEx(playerid);
	SetPlayerSkin(playerid, GetPlayerSkin(playerid));
