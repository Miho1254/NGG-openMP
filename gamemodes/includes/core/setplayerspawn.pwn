SetPlayerSpawn(playerid)
{
    if(IsPlayerConnected(playerid))
	{
		...
		SetPVarInt(playerid, "AntiTPCheck", gettime()+8);
		...
		if(PlayerInfo[playerid][pJailTime] > 0)
		{
		    ...
		    else if(strfind(PlayerInfo[playerid][pPrisonReason], "[OOC]", true) != -1)
		    {
				...
				new string[128];
				format(string, sizeof(string), "Ban dang o tu, vi ly do: %s", PlayerInfo[playerid][pPrisonReason]);
		...
		if(GetPVarInt(playerid, "Injured") == 1)
		{
		    switch(aLastShotWeapon[playerid])
			{
				...
				{
					format(szMiscArray, sizeof(szMiscArray), "Nguoi choi da bi danh bat tinh boi %s.", Weapon_ReturnName(aLastShotWeapon[playerid]));
				}
				case WEAPON_KATANA, WEAPON_KNIFE:
				{
					format(szMiscArray, sizeof(szMiscArray), "Nguoi choi da bi chem da man boi %s.", Weapon_ReturnName(aLastShotWeapon[playerid]));
				}
				case WEAPON_GRENADE, WEAPON_ROCKETLAUNCHER, WEAPON_HEATSEEKER, 51:
				{
					format(szMiscArray, sizeof(szMiscArray), "Nguoi choi bi thuong nghiem trong tu mot vu no.");
				}
				case WEAPON_FIREEXTINGUISHER, WEAPON_SPRAYCAN:
				{
					format(szMiscArray, sizeof(szMiscArray), "Nguoi choi bi nghet tho va bi dau doc.");
				}
				case WEAPON_MOLTOV, WEAPON_FLAMETHROWER:
				{
					format(szMiscArray, sizeof(szMiscArray), "Nguoi choi bi bong nghiem trong.");
				}
				...
				{
					format(szMiscArray, sizeof(szMiscArray), "Nguoi choi bi ban trong thuong tai %s boi vu khi %s.", ReturnBoneName(aLastShotBone[playerid]), Weapon_ReturnName(aLastShotWeapon[playerid]));
				}
				case WEAPON_VEHICLE, WEAPON_COLLISION:
				{
					format(szMiscArray, sizeof(szMiscArray), "Nguoi choi da bi gay xuong nghiem trong.");
				}
				case WEAPON_DROWN:
				{
					format(szMiscArray, sizeof(szMiscArray), "Nguoi choi bat tinh vi duoi nuoc.");
				}
				case 0: format(szMiscArray, sizeof(szMiscArray), "Nguoi choi da bi dam chet boi mot ai do");
				default: format(szMiscArray, sizeof(szMiscArray), "Nguoi choi da bi chet khong ro ly do.");
			}
			strcat(szMiscArray, "\n{FF0000}(Nguoi choi dang bi thuong nang).", sizeof(szMiscArray));
		...
		if(GetPVarInt(playerid, "EventToken") == 1)
		{
            ...
            if(EventKernel[EventType] == 4)
			{
				...
				SendClientMessageEx(playerid, COLOR_WHITE, "Ban da tro thanh Zombie! Su dung lenh (/bite) de lay nhiem nguoi khac.");
