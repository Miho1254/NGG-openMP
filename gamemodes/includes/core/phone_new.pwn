...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]) {
	...
	switch(dialogid)
	{
		case DIALOG_PHONE_COLOR:
		{
			...
			SendClientMessage(playerid, COLOR_LIGHTBLUE, "Su dung (/dienthoai) de truy cap dien thoai cua ban. {BBBBBB}(Su dung '/mouse' de lay lai con tro chuot khi vo tinh huy tuong tac cua ban.)");
			return 1;
		}
		...
		case DIALOG_PHONE_CONTACTS:
		{
			...
			switch(listitem)
			{
				case 0: ShowPlayerDialogEx(playerid, DIALOG_PHONE_YPAGES, DIALOG_STYLE_TABLIST_HEADERS, "Dich vu", "Ten\tSo dien thoai\n\
					--> Businesses\tMENU ->\n\
					...
					Farva Car Repair Co.\t1-800-2232 ", "Chon", "Huy");
				...
				case 1: {
					...
					ShowPlayerDialogEx(playerid, DIALOG_PHONE_PAYPHONES, DIALOG_STYLE_TABLIST_HEADERS, "Pay Phones", szMiscArray, "Goi", "Huy");
				}
				case 2:	{
				...
				case 3: ShowPlayerDialogEx(playerid, DIALOG_PHONE_ADDCONTACT, DIALOG_STYLE_INPUT, "Dien thoai | Them lien he", "Nhap ten lien he ban muon them.\nVi du: Rick_Tran", "Them", "Tro lai");
				case 4: {
		...
		case DIALOG_PHONE_YPAGES: {
			...
			switch(listitem) {
				...
				case 0: {
					...
					return ShowPlayerDialogEx(playerid, DIALOG_PHONE_BUSINESSES, DIALOG_STYLE_LIST, "Dich vu | Businesses", "\
						24/7\n\
						Cua hang quan ao\n\
						Nha hang\n\
						Bars\n\
				...
				case 1: callcmd::call(playerid, "18004444");
				case 2: callcmd::call(playerid, "18001800");
				case 3: callcmd::call(playerid, "1738");
				case 4: callcmd::call(playerid, "18008080");
				case 5: callcmd::call(playerid, "18001111");
				case 6: callcmd::call(playerid, "18001020");
				case 7: callcmd::service(playerid, "mechanic");
			}
		...
		case DIALOG_PHONE_BUSINESSES: {
			...
			if(!response) {
				...
				return ShowPlayerDialogEx(playerid, DIALOG_PHONE_YPAGES, DIALOG_STYLE_TABLIST_HEADERS, "Dich vu", "Ten\tSo dien thoai\n\
					--> Businesses\tMENU ->\n\
					SA News\t1-800-1800\n\
					Government - City Hall/White House\t1-800-4444\n\
					BubbaGump Fish 'n Shrimps Shipment Service\t1-800-6666\n\
		...
		case DIALOG_PHONE_CONTACTLIST:
		{
			...
			callcmd::call(playerid, szMiscArray);
			return 1;
		}
		...
		case DIALOG_PHONE_ADDCONTACT1: {
			...
			if(strlen(szMiscArray) > 16) {
				SendClientMessage(playerid, COLOR_YELLOW, "[DIEN THOAI] {DDDDDD} Ten ban nhap qua dai, vui long thu lai.");
				return ShowPlayerDialogEx(playerid, DIALOG_PHONE_ADDCONTACT1, DIALOG_STYLE_INPUT, "Dien thoai | Them lien he", "Nhap so dien thoai ban muon them.", "Them", "Tro ve");
			}
		...
		case DIALOG_PHONE_CAMERA:
		{
			...
			switch(listitem)
			{
				case 0: callcmd::picture(playerid, "");
				case 1: callcmd::selfie(playerid, "");
				case 2: callcmd::selfie(playerid, "");
			}
		}
		case DIALOG_PHONE_SETTINGS:
		{
			...
			switch(listitem)
			{
				case 0:
				{
					ShowPlayerDialogEx(playerid, DIALOG_PHONE_RINGTONES, DIALOG_STYLE_LIST, "Nhac chuong", "\
							Go Go Track!\n\
							Dual Dual!\n\
							Bee Bees\n\
							So many awards!\n\
		...
		case DIALOG_PHONE_RINGTONES:
		{
			...
			ShowPlayerDialogEx(playerid, DIALOG_PHONE_RINGTONES2, DIALOG_STYLE_MSGBOX, "Dien thoai | Nhac chuong", "Bam 'Chon' de xac nhan.\nBam 'Huy' de huy.", "Chon", "Huy");
		}
...
Phone_Businesses(playerid, btype)
{
	...
	ShowPlayerDialogEx(playerid, DIALOG_PHONE_BUSINESSES2, DIALOG_STYLE_TABLIST, "San Andreas | Ban do | Businesses", szMiscArray, "Chon", "Tro ve");
	return 1;
}
...
CMD:trace(playerid, params[]) {
	...
	if(Bit_State(arrPlayerBits[playerid], phone_bitTraceState)) {
		...
		return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban da ngung tim kiem muc tieu.");
	}

	if(PlayerInfo[playerid][pJob] != 1 && PlayerInfo[playerid][pJob2] != 1 && PlayerInfo[playerid][pJob3] != 1) {
		return SendClientMessageEx(playerid, COLOR_GREY, "Ban khong phai la Tham tu.");
	}
	if(gettime() < UsedFind[playerid]) { // || FindTimePoints[playerid] == 0) {
		return SendClientMessageEx(playerid, COLOR_GREY, "Ban dang tim kiem mot ai do, cho mot lat moi co the tiep tuc tim kiem lai.");
	}

	new iNumber;

	if(sscanf(params, "d", iNumber)) return SendClientMessage(playerid, COLOR_GRAD1, "SU DUNG: /trace [so dien thoai]");

	if(iNumber == 0) return SendClientMessageEx(playerid, COLOR_GRAD1, "So dien thoai ban nhap khong hop le.");
	...
	foreach(new i : Player)	{
		...
		if(PlayerInfo[i][pPnumber] == iNumber) {
			...
			if(GetPlayerInterior(i) != 0)
				return SendClientMessageEx(playerid, COLOR_GREY, " Nguoi nay dang trong mot ngoi nha nao do...");
			...
			if(GetPVarInt(playerid, "_SwimmingActivity") >= 1)
				return SendClientMessageEx(playerid, COLOR_GRAD2, "Ban khong the tim kiem nguoi khac trong khi dang boi.");
			...
			else return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong the tim kiem duoc nguoi nay.");
	...
	SendClientMessage(playerid, COLOR_GRAD1, "So dien thoai nay khong hoat dong hoac khong co ai su dung.");
	return 1;
}
...
public Phone_Trace(playerid, i)
{
	...
	if(IsPlayerInDynamicArea(x, arrRadioTowerData[x][rt_iAreaID]))
	{
		...
		switch(fDistance)
		{
			case 2501 .. 3000: szStrength = "{FF0000}Yeu";
			case 1501 .. 2500: szStrength = "{FFFF00}Trung binh";
			case 0 .. 1500: szStrength = "{00FF00}Tot";
		}
	}
	else szStrength = "{FF0000}Khong co";
	...
	if(!IsValidDynamicObject(arrRadioTowerData[x][rt_iObjectID]))
	{
		...
		format(szMiscArray, sizeof(szMiscArray), "%s\n\
			...
			{FFFFFF}So dien thoai %d da duoc xac dinh vi tri: {FFFF00}%s.", szMiscArray, PlayerInfo[i][pPnumber], arrRadioTowerData[x][rt_szName]);
		ShowPlayerDialogEx(playerid, 0, DIALOG_STYLE_MSGBOX, "Tim vi tri so dien thoai", szMiscArray, "-----", "");
		SendClientMessage(playerid, COLOR_YELLOW, "Su dung lai lenh /trace de huy tim kiem.");
...
public Phone_OnGetContacts(iPlayerID)
{
	...
	if(!iRows) return SendClientMessage(iPlayerID, COLOR_GRAD1, "Ban khong coany contacts.");
	...
	if(GetPVarType(iPlayerID, PVAR_PHONEDELCONTACT)) return ShowPlayerDialogEx(iPlayerID, DIALOG_PHONE_CONTACTLISTDEL, DIALOG_STYLE_TABLIST_HEADERS, "Dien thoai | Xoa lien he", szMiscArray, "Xoa", "Tro ve");
	ShowPlayerDialogEx(iPlayerID, DIALOG_PHONE_CONTACTLIST, DIALOG_STYLE_TABLIST_HEADERS, "Dien thoai | Danh ba", szMiscArray, "Goi", "Tro ve");
	return 1;
}
...
{
	if(mysql_errno()) return Phone_Contacts(iPlayerID), SendClientMessage(iPlayerID, COLOR_GRAD1, "Da xay ra loi, vui long thu lai sau!");
	SendClientMessage(iPlayerID, COLOR_YELLOW, "[DIEN THOAI] {DDDDDD} Them lien he thanh cong.");
...
{
	if(mysql_errno()) return SendClientMessage(iPlayerID, COLOR_GRAD1, "Da xay ra loi, vui long thu lai sau!");
	SendClientMessage(iPlayerID, COLOR_YELLOW, "[DIEN THOAI] {DDDDDD} Xoa lien he thanh cong.");
...
public Phone_CheckContacts(iPlayerID, name[])
{
	...
	if(iRows > MAX_CONTACTS) {
		SendClientMessage(iPlayerID, COLOR_GRAD1, "Danh ba cua ban qua nhieu lien he, khong the them nua.");
		DeletePVar(iPlayerID, "PHN_CONTACT");
	}
	ShowPlayerDialogEx(iPlayerID, DIALOG_PHONE_ADDCONTACT1, DIALOG_STYLE_INPUT, "Them lien he | So dien thoai", "Nhap so dien thoai ban muon them.", "Them", "Huy");
	return 1;
}
...
{
	ShowPlayerDialogEx(iPlayerID, DIALOG_PHONE_DIAL, DIALOG_STYLE_INPUT, "Dien thoai | Goi", "Hay nhap so dien thoai ban muon goi.", "Goi", "Tro ve");
}

Phone_Contacts(iPlayerID)
{
	ShowPlayerDialogEx(iPlayerID, DIALOG_PHONE_CONTACTS, DIALOG_STYLE_LIST, "Dien thoai | Address Book", "\
		Dich vu\n\
		Pay Phones\n\
		Danh ba\n\
		Them lien he\n\
...
Phone_Ads(iPlayerID)
{
	callcmd::ads(iPlayerID, "");
}

Phone_Camera(iPlayerID)
{
	ShowPlayerDialogEx(iPlayerID, DIALOG_PHONE_CAMERA, DIALOG_STYLE_LIST, "Dien thoai | Camera", "Camera sau\nCamera truoc\nReset", "Chon", "Tro ve");
}

Phone_Settings(iPlayerID)
{
	ShowPlayerDialogEx(iPlayerID, DIALOG_PHONE_SETTINGS, DIALOG_STYLE_LIST, "Phone | Cai dat", "Nhac chuong\nHinh nen", "Chon", "Tro ve");
}

Phone_Music(iPlayerID)
{
	callcmd::mp3(iPlayerID, "");
}
...
Phone_Map(iPlayerID)
{
	...
	callcmd::mygps(iPlayerID, "");
}

Phone_SAN(iPlayerID)
{
	callcmd::shows(iPlayerID, "");
}

/*
Phone_House(iPlayerID) {

	callcmd::houselistings(iPlayerID, "");
}

Phone_Medic(iPlayerID)
{
	callcmd::call(iPlayerID, "911");
}

Phone_MDC(iPlayerID)
{
	callcmd::mdc(iPlayerID, "");
}
*/

Phone_Events(iPlayerID) {
	SendClientMessageEx(iPlayerID, COLOR_GRAD1, "Tinh nang nay se som ra mat!");
}
...
{
	ShowPlayerDialogEx(iPlayerID, DIALOG_PHONE_WALLPAPERS, DIALOG_STYLE_LIST, "Dien thoai | Hinh nen", "\
		Standard\n\
		Vice City 1\n\
		Vice City 2\n\
		Cop Art 1\n\
...
Phone_PhoneColorMenu(playerid) {
	...
	ShowPlayerDialogEx(playerid, DIALOG_PHONE_COLOR, DIALOG_STYLE_LIST, "Dien thoai - Chon mau", "Mau den\nMau trang\nMau do\nMau hong\nMau cam\nMau nau\nMau vang\nMau xanh la\nMau xanh da troi\nMau xanh sang", "Chon", "");
}
...
Phone_Main(playerid) {
	if(GetPVarInt(playerid, "EventToken")) {
		return SendClientMessageEx(playerid, COLOR_GRAD1, "Lam khong the lam dieu nay trong su kien.");
	}
		...
		return SendClientMessageEx(playerid, COLOR_GRAD2, "Ban dang sua chua phuong tien!");
	}
	if(PlayerCuffed[playerid] != 0 || GetPVarInt(playerid, "pBagged") >= 1 || PlayerInfo[playerid][pHospital] != 0) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong the su dung dien thoai ngay bay gio.");
	if(PlayerInfo[playerid][pPnumber] == 0) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong co dien thoai.");
	if(!Bit_State(arrPlayerBits[playerid], phone_bitState)) {
		...
		SendClientMessageEx(playerid, COLOR_YELLOW, "[DIEN THOAI]{DDDDDD} Su dung ~k~~GO_FORWARD~ de di chuyen len, ~k~~GO_BACK~ de di chuyen xuong ~k~~PED_SPRINT~ de chon.");
...
CMD:selfie(playerid, params[]) {
	...
	if(IsPlayerInAnyVehicle(playerid)) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong the lam dieu nay khi o trong phuong tien.");
	if(GetPVarInt(playerid, "EventToken")) {
	...
	if(!Bit_State(arrPlayerBits[playerid], phone_bitCamState))
	{
	    ...
	    SendClientMessage(playerid, COLOR_GRAD1, "{DDDDDD}Su dung (/selfie) de quay tro lai. Nhap {FFFF00}/headmove {DDDDDD}de co dinh khuon mat (nhin thang).");
	    Bit_On(arrPlayerBits[playerid], phone_bitCamState);
	}
	else
	{
...
Phone_InitTD(playerid) {
	...
	*/
	...
	phone_PTextDraw[playerid][6] = CreatePlayerTextDraw(playerid,571.000000, 376.000000, "Huy");
	...
	phone_PTextDraw[playerid][11] = CreatePlayerTextDraw(playerid,518.000000, 376.000000, "Chon");
	...
	phone_PTextDraw[playerid][12] = CreatePlayerTextDraw(playerid,510.000000, 266.000000, "Goi dien");
	...
	phone_PTextDraw[playerid][13] = CreatePlayerTextDraw(playerid,510.000000, 278.000000, "Danh ba");
	...
	phone_PTextDraw[playerid][14] = CreatePlayerTextDraw(playerid,510.000000, 290.000000, "Su kien");
	...
	phone_PTextDraw[playerid][15] = CreatePlayerTextDraw(playerid,510.000000, 302.000000, "Quang cao");
	...
	phone_PTextDraw[playerid][16] = CreatePlayerTextDraw(playerid,510.000000, 314.000000, "Nhac");
	...
	phone_PTextDraw[playerid][18] = CreatePlayerTextDraw(playerid,510.000000, 338.000000, "Tim dia diem");
	...
	phone_PTextDraw[playerid][20] = CreatePlayerTextDraw(playerid,510.000000, 362.000000, "Cai dat");
