		...
		case DIALOG_AMMUNATION_AMMO:
		{
			...
			switch(listitem)
			{
				case 0: // Pistol Ammo
				{
					...
					if(PlayerInfo[playerid][pCash] < GunPrices[3]) return SendClientMessage(playerid, COLOR_WHITE, "Ban khong du tien on you.");
				...
				case 1: // Shotgun Ammo
				{
					...
					if(PlayerInfo[playerid][pCash] < GunPrices[4]) return SendClientMessage(playerid, COLOR_WHITE, "Ban khong du tien on you.");
				...
				case 2: // Deagle ammo
				{
					...
					if(PlayerInfo[playerid][pCash] < GunPrices[5]) return SendClientMessage(playerid, COLOR_WHITE, "Ban khong du tien on you.");
				...
				case 3: // Rifle ammo
				{
					...
					if(PlayerInfo[playerid][pCash] < GunPrices[6]) return SendClientMessage(playerid, COLOR_WHITE, "Ban khong du tien on you.");
...
CMD:seeammo(playerid, params[]) {
	...
	if(!IsPlayerConnected(target)) return SendClientMessageEx(playerid, COLOR_GRAD2, "Nguoi nay khong ket noi");
...
CMD:setammo(playerid, params[]) {
	...
	if(!IsPlayerConnected(iTargetID)) return SendClientMessageEx(playerid, COLOR_GRAD2, "Nguoi nay khong ket noi.");
	if(iAmount < 1) return SendClientMessageEx(playerid, COLOR_GRAD2, "The amount cannot be less than 1.");	

	format(szMiscArray, sizeof(szMiscArray), "Ban da cho %s %i ammo (type:%i)", GetPlayerNameEx(iTargetID), iAmount, iAmmoType);
	SendClientMessageEx(playerid, COLOR_WHITE, szMiscArray);
	
	format(szMiscArray, sizeof(szMiscArray), "%s(%s) da duoc cho %s(%d) (%s) %i ammo (type:%i)", GetPlayerNameEx(playerid), GetPlayerIpEx(playerid), GetPlayerNameEx(iTargetID), GetPlayerSQLId(iTargetID), GetPlayerIpEx(iTargetID), iAmount, iAmmoType);
...
CMD:vipgunsleft(playerid, params[]) {
	...
	szMiscArray[0] = 0;
	if(PlayerInfo[playerid][pDonateRank] < 1) return SendClientMessageEx(playerid, COLOR_GRAD2, "Ban khong phai la VIP member.");
...
/*CMD:oissuegl(playerid, params[]) return callcmd::oissuegunlicense(playerid, params);
CMD:oissuegunlicense(playerid, params[])
{
	if((0 <= PlayerInfo[playerid][pLeader] < MAX_GROUPS) && arrGroupData[PlayerInfo[playerid][pLeader]][g_iGroupType] == GROUP_TYPE_GOV)
	{
		...
		if(IsPlayerConnected(ReturnUser(TargetName))) return callcmd::issuegunlicense(playerid, params);
...
CMD:orevokegl(playerid, params[]) return callcmd::orevokegunlicense(playerid, params);
...
public OnOfflineGunLicense(playerid, task, name[])
{
	if(cache_get_row_count(MainPipeline) == 0)
		return SendClientMessageEx(playerid, COLOR_RED, "Error - This account khong ton tai.");
