...
hook OnGameModeInit() {
	...
	UpdateIn[1] = TextDrawCreate(595.181335, 431.007720, " ");
	...
	switch(dialogid)
	{
		case DIALOG_MAINTENANCE:
		{
			if(response) {
				...
				if(PlayerInfo[playerid][pAdmin] < 1337) return SendClientMessageEx(playerid, COLOR_GRAD2, "Ban khong co du quyen de thuc hien lenh nay");
				server_restaring=true;
			    SendClientMessageToAllEx(COLOR_LIGHTBLUE, "* May chu se bao tri trong vai phut toi, (Xem thoi gian se bao tri o phia goc man hinh)");
			    GameTextForAll("~n~~n~~n~~n~~y~] Bao Tri May Chu ]", 5000, 3);
...
{
    if(PlayerInfo[playerid][pAdmin] < 1337) return SendClientMessageEx(playerid, COLOR_GRAD2, "Ban khong duoc phep su dung lenh nay.");
...
public MaintenanceTimer() {
	...
	if(SystemUpdate == 15) GameTextForAll("~n~~n~~n~~n~~w~HAY ~r~THOAT GAME ~w~de khong bi rollback ~y~Du lieu tai khoan ~w~da duoc ~g~luu~w~!", 5000, 3);
