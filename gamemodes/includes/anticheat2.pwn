...
public OnPlayerWeaponShot(playerid, weaponid, hittype, hitid, Float:fX, Float:fY, Float:fZ) {
	...
	if(weaponid == WEAPON_SILENCED && pTazer{playerid} == 1) {
		...
		if(iShots < 1) {
			...
			SendClientMessageEx(playerid, COLOR_WHITE, "Tazer cua ban dang sac!");
...
stock AC_InitWeaponData() {
	...
	arrWeaponData[22][ac_WeaponDamage] = 15.0; // 22 - 9mm
	arrWeaponData[23][ac_WeaponDamage] = 15.0; // 23 - Silenced 9mm
	arrWeaponData[24][ac_WeaponDamage] = 46.2; // 24 - Deagle
	arrWeaponData[25][ac_WeaponDamage] = 3.3; // 25 - Shotgun (per pellet)
	...
	//if(arrAntiCheat[playerid][ac_iFlags][processid] == 1 || arrAntiCheat[playerid][ac_iFlags][processid] % 5 == 0) { // prevent spamming
	{
		...
		switch(processid) {
			...
			case AC_CMDSPAM: {
				...
				if(iExtraID > 15) {
					SendClientMessageEx(playerid, COLOR_YELLOW, "Ba da bi tat tieng khi go lenh.");
					format(szMiscArray, sizeof(szMiscArray), "{AA3333}GTN-Warning{FFFF00}: %s is spamming commands (%d times).", GetPlayerNameEx(playerid), arrAntiCheat[playerid][ac_iCommandCount]);
				}
				else return SendClientMessageEx(playerid, COLOR_YELLOW, "Ba da bi tat tieng khi go lenh.");
			}
			case AC_CARSURFING: {
...
AC_KeySpamCheck(playerid) {
	if(GetPVarType(playerid, "PCMute")) {
		SendClientMessageEx(playerid, COLOR_WHITE, "[SYSTEM]: Hien tai ban dang bi chan su dung cac phim tuong tac.");
		return 0;
	}
	if(ac_iPlayerKeySpam[playerid] > 4) {
		SetPVarInt(playerid, "PCMute", 1);
		defer AC_ResetPVars[10000](playerid, 1);
		SendClientMessageEx(playerid, COLOR_LIGHTRED, "[SYSTEM]: Ban da bi tat tieng vi spam phim tuong tac . Khong duoc lap lai.");
		return 0;
	}
	return 1;
}
...
CMD:listacflags(playerid, params[]) {
	...
	if(PlayerInfo[playerid][pAdmin] >= 2) {
		...
		if(PlayerInfo[playerid][pAdmin] >= 4) SendClientMessage(playerid, COLOR_GRAD2, "NOTE: Neu ban chac chan nguoi choi do khong dung hack co the su dung /resetpaflags.");
	}
