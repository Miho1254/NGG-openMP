...
{
   /* // BẮT ĐẦU: ĐOẠN CODE CHẶN IP DO NGƯỜI DÙNG CUNG CẤP
    new ipname[64];
    new const ATTACKER_SUBNET[] = "125.235.209."; // Dải IP cần chặn
    ...
    // strfind(pIP, ATTACKER_SUBNET, true) sẽ trả về 0 nếu pIP bắt đầu bằng ATTACKER_SUBNET
    if (strfind(name, "ddos", true) == 0 || strfind(name, "dos", true) == 0 || strfind(name, "tao", true) == 0 || strfind(name, "ddos", true) == 0 || strfind(ipname, ATTACKER_SUBNET, true) == 0 || strfind(ipname, ATTACKER_SUBNET2, true) == 0 || strfind(ipname, ATTACKER_SUBNET3, true) == 0 || strfind(ipname, ATTACKER_SUBNET4, true) == 0) 
    {
        printf("[ATTACKER BLOCK] BANNED SUBNET %s for player %s.", ATTACKER_SUBNET, ipname);
        
        // Block vinh vien trong he thong SA-MP (10 nam)
        BlockIpAddress(ipname, 60 * 60 * 24 * 365 * 10); 
        
        Kick(playerid);
        return 0; // QUAN TRỌNG: Ngăn chặn toàn bộ xử lý tiếp theo của RPC JoinGame
    }
    // KẾT THÚC: ĐOẠN CODE CHẶN IP*/
